...
export default function Overview() {
  ...
  const { data: summary, loading, error, reload } = useFetchData(fetchSummary)
  const { data: hotspots } = useFetchData(() => fetchHotspots(100))
  const { data: opportunities } = useFetchData(() => fetchOpportunities(1))

  useEffect(() => {
    if (opportunities?.opportunities?.[0]) {
      setTopOpportunity(opportunities.opportunities[0])
    }
  }, [opportunities, setTopOpportunity])
  ...
  const temporalData = Object.entries(summary.temporal_distribution || {})
    .sort((a, b) => Number(a[0]) - Number(b[0]))
    .map(([month, count]) => ({ month: monthNames[month] || `M${month}`, violations: count }))

  // Data quality bars — all from backend fields
  const totalMissingCols = Object.keys(summary.missing_values || {}).length
  const totalCols = summary.total_columns || 1

  const qualityBars = [
    {
      label: 'Columns with Missing Data',
      value: Math.min((totalMissingCols / totalCols) * 100, 100),
      color: '#F59E0B',
      note: `${totalMissingCols} of ${totalCols} cols`,
    },
    {
      label: 'Named Junction Coverage',
      value: summary.total_records > 0
        ? Math.min((summary.total_junction_records / summary.total_records) * 100, 100)
        : 0,
      color: '#10B981',
      note: `${formatNumber(summary.total_junction_records)} records`,
    },
    {
      label: 'Data Retained After Filter',
      value: summary.original_records > 0
        ? Math.min((summary.total_records / summary.original_records) * 100, 100)
        : 100,
      color: '#00D4FF',
      note: `${formatNumber(summary.total_records)} kept`,
    },
  ]

  // Vehicle type chart — from backend vehicle_types
  const vehicleData = Object.entries(summary.vehicle_types || {})
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map(([name, count]) => ({ name, count }))

  // Top opportunity score from backend
  const topOppScore = opportunities?.opportunities?.[0]?.opportunity_score ?? null

  return (
    <div className="space-y-6">
      {/* KPI Row — all backend fields */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="lg:col-span-2 glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="badge-blue">{hotspots?.total_hotspots || 0} hotspots</span>
          </div>
          <HotspotMap
            markers={hotspots?.hotspots || []}
            height="420px"
            renderPopup={(m) => `
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-5">
          ...
          {/* Data quality issues from backend */}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={temporalData}>
              ...
              <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v) => [formatNumber(v), 'Violations']} />
      ...
      {/* Bottom KPI row — all from summary */}
